# -*- coding: utf-8 -*-
"""
Created on Thu May 23 16:48:47 2024
...
parent_folder = "./data/CK/"  # Path to the parent folder containing 30 class folders
images, labels = load_images_from_folders(parent_folder)
train_images, test_images, train_labels, test_labels = train_test_split(images, labels, test_size=0.2, random_state=42)
train_images, val_images, train_labels, val_labels = train_test_split(train_images, train_labels, test_size=0.1, random_state=42)

# Apply wavelet-based filtering to images
def apply_wavelet_filter(images):
    processed_images = []
    for img in images:
        coeffs = pywt.dwt2(img, 'haar')  # Apply 2D discrete wavelet transform
        LL, (LH, HL, HH) = coeffs
        processed_img = pywt.idwt2((LL*0, (LH, HL, HH)), 'haar')  # Reconstruct the image using only the high-frequency components
        processed_img = processed_img[:, :, :3]  # Ensure only the first 3 channels are retained
        processed_images.append(processed_img)
    return np.array(processed_images)

train_images_wavelet = apply_wavelet_filter(train_images)
val_images_wavelet = apply_wavelet_filter(val_images)
test_images_wavelet = apply_wavelet_filter(test_images)
...
for i in range(num_runs):
    ...
    acc = train_and_evaluate_model(train_images_wavelet, train_labels, val_images_wavelet, val_labels, test_images_wavelet, test_labels)
    results.append(acc)
